...
**Google login:** In [Google Cloud Console](https://console.cloud.google.com/apis/credentials) create an OAuth 2.0 Client ID (Web application). Add redirect URI `http://localhost:3000/api/auth/google/callback` (and your production URL with **https** if you deploy the admin). Put the Client ID and secret in `.env` as `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET`. You **must** set `ALLOWED_ADMIN_EMAILS=your@gmail.com` (comma-separated); only those emails can log in, and no one can enter until this is set.
