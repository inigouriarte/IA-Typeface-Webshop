...
function processFontFiles(uploadedFiles, dirName) {
  const weightMap = {};
  const fontFiles = [];
  ...
  return { weightMap, fontFiles, weights: Object.values(weightMap) };
}

function buildFontFaceCSS(weights, dirName, fontName) {
  let css = '';
...
module.exports = async function (req, res) {
  ...
  try {
    ...
    return await handleCreate(fields, uploadedFiles, res);
  } catch (e) {
    console.error('Font operation error:', e);
...
async function handleCreate(fields, uploadedFiles, res) {
  ...
  const { fontFiles, weights } = processFontFiles(uploadedFiles, dirName);
  if (!weights.length) {
  ...
  let fontFaceCSS = `\n/* ${name} Font Faces */\n` + buildFontFaceCSS(weights, dirName, name);
  ...
  // 7. Update shared api/_font-dirs.js
...
async function handleUpdate(fields, uploadedFiles, res) {
